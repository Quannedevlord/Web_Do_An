<?php
// playlist_view.php – Trang xem playlist cụ thể
session_start();
...
$plRow = null;
$check = mysqli_query($conn, "SELECT * FROM playlists WHERE id=$playlistId AND user_id=$userId");
if(!$check || !mysqli_num_rows($check)){ header("Location: index.php"); exit; }
...
$plName = htmlspecialchars($plRow['name']);
?>
<!DOCTYPE html>
<html lang="vi">
<head>
<meta charset="utf-8"/><meta name="viewport" content="width=device-width,initial-scale=1.0"/>
<title><?=$plName?> – Chill Wave</title>
...
.glass-player{background:rgba(255,255,255,0.85);backdrop-filter:blur(16px);}
.glass-player{background:rgba(255,255,255,0.85);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);}
.song-row.playing td,.song-row.playing span{color:#42a7f0 !important;}
.eq-icon{color:#42a7f0;animation:pulse .8s ease-in-out infinite alternate;}
@keyframes pulse{from{opacity:.5}to{opacity:1}}
.pl-song-grid{display:grid;align-items:center;gap:0.35rem;grid-template-columns:28px minmax(0,1fr) 32px;}
@media (min-width:640px){
  .pl-song-grid{gap:0.5rem;grid-template-columns:40px minmax(0,1fr) 40px;}
}
</style>
</head>
...
<aside id="sidebar" class="w-60 max-w-[min(100vw,15rem)] flex flex-col bg-white/60 border-r border-slate-200/60 backdrop-blur-sm p-4 shrink-0 overflow-y-auto">
    ...
    <div class="bg-white/50 rounded-xl p-3 flex-1">
        <div class="flex items-center justify-between mb-3 px-1">
            ...
            <button onclick="Library.createPlaylist()" class="text-white/50 hover:text-white" title="Tạo playlist">
                <span class="material-symbols-outlined text-[20px]">add</span>
            </button>
        </div>
        <a href="liked.php" class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/10 transition-colors mb-1">
            <div class="size-10 rounded-lg bg-gradient-to-br from-purple-600 to-blue-400 flex items-center justify-center">❤️</div>
            <div><p class="text-sm font-medium text-white">Liked Songs</p><p class="text-[11px] text-slate-400">Playlist</p></div>
        </a>
    ...
    <div class="mt-4 flex items-center gap-3 px-2 py-2">
        ...
        <span class="text-sm text-slate-700 flex-1 truncate"><?=$username?></span>
        <a href="logout.php" class="text-slate-400 hover:text-red-400"><span class="material-symbols-outlined text-[18px]">logout</span></a>
    </div>
</aside>
...
<main class="flex-1 min-w-0 overflow-y-auto pb-28 sm:pb-32">
    ...
    <div class="bg-gradient-to-br from-primary/20 via-cream to-sage/20 px-4 sm:px-8 pt-6 sm:pt-10 pb-6 sm:pb-8 rounded-2xl sm:rounded-3xl mx-4 sm:mx-6 mt-4 sm:mt-6 border border-white/50 shadow-sm">
        <div class="flex items-start gap-2 sm:gap-3">
            ...
            <div class="size-32 sm:size-48 rounded-xl bg-gradient-to-br from-primary to-blue-600 flex items-center justify-center text-4xl sm:text-6xl shadow-2xl flex-shrink-0 mx-auto sm:mx-0">🎵</div>
            <div class="text-center sm:text-left min-w-0 flex-1">
                ...
                <h1 class="text-2xl sm:text-4xl md:text-5xl font-black text-slate-900 mb-2 sm:mb-4 break-words"><?=$plName?></h1>
                <p class="text-sm text-slate-500 truncate"><?=$username?> · <span id="plSongCount">0 bài hát</span></p>
            </div>
            </div>
        </div>
    </div>
    ...
    <!-- Controls -->
    <div class="px-4 sm:px-8 py-3 sm:py-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
        <button id="playAllBtn" class="size-12 sm:size-14 rounded-full bg-primary text-white flex items-center justify-center shadow-xl shadow-primary/30 hover:bg-blue-500 hover:scale-105 transition-all shrink-0 self-start">
            <span class="material-symbols-outlined text-[24px] sm:text-[28px]" style="font-variation-settings:'FILL' 1">play_arrow</span>
        </button>
        <button type="button" onclick="Library.deletePlaylist(<?=$playlistId?>)"
                class="sm:ml-auto flex items-center justify-center gap-2 text-red-400 hover:text-red-600 text-xs sm:text-sm border border-red-200 px-3 sm:px-4 py-2 rounded-xl hover:bg-red-50 transition-colors w-full sm:w-auto">
            <span class="material-symbols-outlined text-[18px]">delete</span>Xóa playlist
        </button>
    </div>
    ...
    <div class="px-4 sm:px-8">
        ...
        <div class="pl-song-grid border-b border-slate-200 pb-2 mb-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider text-slate-400">
            <span class="text-center">#</span>
            <span>Tiêu đề</span>
            <span></span>
        </div>
        <div id="plSongList"><p class="text-center text-slate-400 py-16">⏳ Đang tải...</p></div>
        </div>
        </div>
    </div>
</main>
...
<div class="fixed bottom-2 sm:bottom-4 left-1/2 -translate-x-1/2 w-[calc(100%-1rem)] sm:w-[94%] max-w-[1400px] min-h-[4.75rem] md:h-24
    ...
    <div class="flex items-center justify-end gap-1 sm:gap-3 shrink-0 md:w-1/4">
        <button id="queueBtn" type="button" class="text-slate-400 hover:text-primary p-0.5" title="Hàng chờ"><span class="material-symbols-outlined text-[20px]">queue_music</span></button>
        <div class="hidden md:flex items-center gap-2">
            <span class="material-symbols-outlined text-slate-400 text-[18px]">volume_up</span>
            <div id="volumeTrack" class="w-16 lg:w-20 h-1 sm:h-1.5 bg-slate-200 rounded-full relative cursor-pointer">
...
document.addEventListener('DOMContentLoaded',()=>{
    ...
    // Load playlist songs
    fetch(`getPlaylistSongs.php?id=${PLAYLIST_ID}`).then(r=>r.json()).then(data=>{
        const songs=data.songs||[];
        window.__playerSongs=songs;
        const list=document.getElementById('plSongList');
        const cnt=document.getElementById('plSongCount');
        if(cnt)cnt.textContent=songs.length+' bài hát';

        if(!songs.length){
            list.innerHTML='<p class="text-center text-slate-400 py-16">Playlist này chưa có bài. Về trang chủ và bấm ➕ để thêm!</p>';
            return;
        }

        list.innerHTML=songs.map((s,i)=>`
            <div class="song-row pl-song-grid items-center py-2.5 sm:py-3 px-1 sm:px-3 rounded-xl cursor-pointer transition-colors hover:bg-white/80 group"
                 data-index="${i}" onclick="Player.selectSong(${i})">
                ...
                <button type="button" onclick="event.stopPropagation();removeSong(${s.id},this)"
                        class="text-slate-300 hover:text-red-400 transition-colors opacity-100 sm:opacity-40 sm:group-hover:opacity-100 justify-self-end">
...
function removeSong(songId,btn){
    ...
    fetch('removeFromPlaylist.php',{method:'POST',headers:{'Content-Type':'application/json'},
          body:JSON.stringify({playlist_id:PLAYLIST_ID,song_id:songId})})
        .then(r=>r.json())
        .then(data=>{
            if(data.success){
                showPopup(data.msg,'info');
                btn.closest('.song-row').style.opacity='0';
                setTimeout(()=>btn.closest('.song-row').remove(),300);
